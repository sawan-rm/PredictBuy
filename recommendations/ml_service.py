...
class RecommendationEngine:
    ...
    def prepare_data(self):
        ...
        print(f"   Found {len(self.users_list)} users and {len(self.products_list)} products")
        ...
        for interaction in interactions:
            ...
            weight = self.interaction_weights.get(interaction.interaction_type, 1)
            matrix[user_idx, product_idx] += weight
        
        self.user_product_matrix = matrix
        print(f"   Created matrix: {matrix.shape[0]} users × {matrix.shape[1]} products")
        return self
    ...
    def get_recommendations(self, user_id, num_recommendations=10, exclude_viewed=True):
        ...
        top_indices = np.argsort(recommendation_scores)[::-1][:num_recommendations]
